...
//{
//	return proxy.originalSystemCall( SP_REGISTER_SERVER_CMD, package );
//}
...
float trap_AAS_Time(void) {
	int temp;
	temp = proxy.originalSystemCall(BOTLIB_AAS_TIME);
	return (*(float*)& temp);
}
...
float trap_Characteristic_Float(int character, int index) {
	int temp;
	temp = proxy.originalSystemCall(BOTLIB_AI_CHARACTERISTIC_FLOAT, character, index);
	return (*(float*)& temp);
}

float trap_Characteristic_BFloat(int character, int index, float min, float max) {
	int temp;
	temp = proxy.originalSystemCall(BOTLIB_AI_CHARACTERISTIC_BFLOAT, character, index, PASSFLOAT(min), PASSFLOAT(max));
	return (*(float*)& temp);
}
...
float trap_BotAvoidGoalTime(int goalstate, int number) {
	int temp;
	temp = proxy.originalSystemCall(BOTLIB_AI_AVOID_GOAL_TIME, goalstate, number);
	return (*(float*)& temp);
}
...
void trap_BotMutateGoalFuzzyLogic(int goalstate, float range) {
	proxy.originalSystemCall(BOTLIB_AI_MUTATE_GOAL_FUZZY_LOGIC, goalstate, range);
}
...
{ //rwwRMG - added [NEWTRAP]
	proxy.originalSystemCall(G_SET_ACTIVE_SUBBSP, index);
}
...
{ //rwwRMG - added [NEWTRAP]
	proxy.originalSystemCall(G_RMG_INIT, terrainID);
}
